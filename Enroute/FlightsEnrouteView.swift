...
struct FlightsEnrouteView: View {
    @Environment(\.managedObjectContext) var context    // Not necessary in modern versions of swift
    ...
    var filter: some View {
        ...
        .sheet(isPresented: $showFilter) {
            FilterFlights(flightSearch: self.$flightSearch, isPresented: self.$showFilter)
                .environment(\.managedObjectContext, context)   // could be commented out
        }
    }
...
//    let airport = Airport.withICAO("KSFO", context: PersistenceController.shared.container.viewContext)
    
    static var previews: some View {
        FlightsEnrouteView(flightSearch: FlightSearch(destination: Airport.withICAO("KSFO", context: PersistenceController.shared.container.viewContext)))
    }
}
